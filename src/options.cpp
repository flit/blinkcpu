...
Options::parse_longopt(OptIter & iter, const char * & optarg) {
   ...
   if (optspec.isValRequired() &&  !(optctrls & Options::QUIET)) {
      ...
      int  longopt_len;
      if (spc) {
         longopt_len = static_cast<int>(spc - longopt);
      } else {
         longopt_len = static_cast<int>(::strlen(longopt));
      }
...
Options::usage(ostream & os, const char * positionals) const {
   ...
   os << "usage: " << cmdname ;
   unsigned  ll = static_cast<unsigned>(strlen(cmdname)) + 7;
   ...
   for (nloop = 0, first = 1 ; !nloop ; optv++, first = 0) {
      ...
      if (! *optv) {
         len = static_cast<unsigned>(strlen(positionals));
